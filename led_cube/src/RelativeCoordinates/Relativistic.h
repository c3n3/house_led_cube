...
namespace Relativistic
{
    ...
    void setLED(ui8 x, ui8 y, ui8 z, Directions direction, uint8_t power);
    
    // set a relative LED coordinate to a color, but also make sure it is protected
    void protectedSetLED(ui8 x, ui8 y, ui8 z, Directions direction, uint8_t color);
    ...
    void moveRow(uint8_t firstCoord, uint8_t secondCoord, Relativistic::Directions viewPoint, uint16_t specificColor = 1, boolean collective = false, uint8_t start = 0, uint8_t end = AXIS_MAX, uint8_t power = 1);
