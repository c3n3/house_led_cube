...
uint8_t TypeEngine::upperHeight = 3;
...
void TypeEngine::drawChar(char c, int x1, int y1, int z1, Relativistic::Directions viewpoint, uint16_t color)
{
    ...
    for (int j = 4; j >= 0; j--)
    {
        for (int i = 0; i < 3; i++)
        {
            ...
            Relativistic::setDirectionalLED(x1 + i, y1 + j, z1, viewpoint, on ? color : 0x0000);
        }
    }
}

void TypeEngine::typeUpper(char c, light_t color) {
    ...
    if (c == 'e') {
        ...
        type('\0', upperCharCount, color);
        return;
    }
    type(c, upperCharCount, color);
    upperChars[upperCharCount] = c;
    upperCharCount++;
}

void TypeEngine::clearLayer() {
    for (int i = 0; i < charsPerSide * 4; ++i) {
        type('\0', i, 0x0000); // bassically type nothing in all the places to clear
    }
}
...
void TypeEngine::type(char c, light_t position, light_t color)
{
    Relativistic::Directions direction = getDirection(position);
    uint8_t x = (position % charsPerSide) * charWidth;
    uint8_t y = upperHeight;
